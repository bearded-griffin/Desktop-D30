...
void DrawMainMenu(Project &project) {
    ...
    if (ImGui::BeginMainMenuBar()) {
        ...
        if (ImGui::BeginMenu("File")) {
            ...
            if (ImGui::MenuItem("Load CSV Data...")) {
                auto selection = pfd::open_file("Select CSV", ".", { "CSV Files", "*.csv" }).result();
                if (!selection.empty()) {
        ...
        if (ImGui::BeginMenu("Printer")) {
            ...
            if (Printer::Get().IsConnected()) {
                ImGui::TextColored(ImVec4(0, 1, 0, 1), "Connected: %s", Printer::Get().GetConnectedName().c_str());
    ...
    if (ImGui::BeginPopupModal("DeviceScanPopup", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    if (ImGui::BeginPopupModal("BatchPrintPopup", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
        static int startRow = 1;
        static int endRow = 0;
        
        // Initialize endRow once when opening
        if (endRow == 0 && !project.csvRows.empty()) endRow = (int)project.csvRows.size();
        ...
        if (endRow > (int)project.csvRows.size()) endRow = (int)project.csvRows.size();
...
void DrawSidebar(Project &project, int &selectedIndex) {
    ...
    if (selectedIndex >= 0 && selectedIndex < project.objects.size()) {
        ...
        if (!project.csvHeaders.empty() && 
           (obj.type == ObjectType::Text || obj.type == ObjectType::Field || obj.type == ObjectType::QRCode)) {
            
            ImGui::Spacing();
            ImGui::Separator();
            ImGui::Text("Data Binding (Batch)");
            
            std::string currentLink = obj.linkedColumn.empty() ? "[None]" : obj.linkedColumn;
