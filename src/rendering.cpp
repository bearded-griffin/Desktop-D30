...
void RenderQRCode(LabelObject &obj, const Color &col) {
      ...
      if (obj.type == ObjectType::Text || obj.type == ObjectType::Field) {
        ...
        if (FileExists(obj.data.c_str())) {
          ...
          ImageDrawRectangleLines(
              &canvas, {obj.x, obj.y, obj.width, obj.height}, 2, BLACK);
          ImageDrawText(&canvas, "FILE NOT FOUND", (int)obj.x + 5,
                        (int)obj.y + 5, 10, BLACK);
        }
        // Render Basic Shapes
   ...
   * @author   bearded.griffin
   ****************************************************/
  void ImageDrawRoundedRectFilled(Image * dst, float x, float y, float w,
                                  float h, float radius, Color col) {
    if (radius <= 0) {
    ...
    ImageDrawRectangle(dst, (int)x, (int)(y + r), (int)w, (int)(h - 2 * r),
                       col);
    // Vertical bar (covers top to bottom, inset by radius width)
    ImageDrawRectangle(dst, (int)(x + r), (int)y, (int)(w - 2 * r), (int)h,
                       col);
  }
   ...
   * @author   bearded.griffin
   ****************************************************/
  float DrawTextBox(Image * target, Font font, const char *text, float x,
                    float y, float fontSize, float spacing, Color tint,
                    float maxWidth) {
    if (maxWidth <= 0) {
    ...
    for (const auto &word : words) {
      ...
      if (target) {
        ...
        ImageDrawTextEx(target, font, word.c_str(),
                        {x + currentX, y + currentY}, fontSize, spacing, tint);
      } else {
   ...
   ****************************************************/
  void RenderScene(
      Project & currentProject, const InteractionState &interactionState,
